...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  const canUpdate = await hasPermission(env.DB, actor.id, "emprendedores", "update");
  if (!canUpdate) {
    return json({ ok: false, message: "No tienes permisos para modificar la configuración de emprendedores." }, 403);
  }
  ...
  for (const lang of siteLanguages) {
    ...
    const title = String(form.get(`title_${loc}`) || "Emprendedores afro").trim();
    const lead =
      String(form.get(`lead_${loc}`) || "").trim() ||
      "Un directorio para descubrir y apoyar negocios de la diáspora — comprar afro también es activismo económico.";
    const band_title = String(form.get(`band_title_${loc}`) || "¿Tienes un emprendimiento?").trim();
    const band_dek = String(form.get(`band_dek_${loc}`) || "Súmalo al directorio — es gratuito para la comunidad.").trim();
    const band_cta_label = String(form.get(`band_cta_label_${loc}`) || "Sumar mi negocio").trim();
    ...
    await env.DB.prepare(`
      INSERT INTO entrepreneurs_page_locales (
  ...
  return json({ ok: true, message: "Configuración de la página de Emprendedores guardada con éxito." });
};
