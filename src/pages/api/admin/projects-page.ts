...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  const canUpdate = await hasPermission(env.DB, actor.id, "proyectos", "update");
  if (!canUpdate) {
    return json({ ok: false, message: "No tienes permisos para modificar la configuración de proyectos." }, 403);
  }
  ...
  for (const lang of siteLanguages) {
    ...
    const title = String(form.get(`title_${loc}`) || "Proyectos en territorio").trim();
    const lead = String(form.get(`lead_${loc}`) || "").trim();
    const band_title = String(form.get(`band_title_${loc}`) || "¿Tienes una idea para tu comunidad?").trim();
    const band_dek = String(form.get(`band_dek_${loc}`) || "Propón un proyecto y construyámoslo juntos.").trim();
    const band_cta_label = String(form.get(`band_cta_label_${loc}`) || "Proponer proyecto").trim();
    ...
    await env.DB.prepare(`
      INSERT INTO projects_page_locales (
  ...
  return json({ ok: true, message: "Configuración de la página de Proyectos guardada con éxito." });
};
