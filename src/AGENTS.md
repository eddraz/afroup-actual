...
- Keep admin Studio page chrome in the shared kit `components/admin/`: `AdminPageHeader` (title h2, count badge, lead, actions slot), `AdminToolbar` (search input + filter join buttons via `filterName`/`filterAttr`/`filterClass` + right/left-extra slots), `AdminTable` (rounded-3xl zebra shell, columns prop, rows/empty slots), `AdminStatCard` (label/value/icon/tone/href/footNote), and `AdminDrawerPanel` (`toggleId`, width `form`|`canvas`, content/default/actions slots). Admin pages under `pages/admin/` and `pages/en/admin/` render header metrics, live search/filter toolbars, rounded data tables, and drawer shells through these components instead of hand-written markup; page scripts own all JS-coupled selectors and rows stay page-owned inside the table's rows slot. Module components (`ReferentesAdmin`, `ResourcesAdmin`, `CollaborateAdmin`, `ContactAdmin`) use `AdminPageHeader` + `AdminStatCard` and canonical search-input classes; where their inline JS binds attributes AdminToolbar cannot render (e.g. `[data-search-input]`, dual filter groups, `btn-active` toggling), that toolbar stays hand-written with canonical classes.
...
- Keep language switching inside admin drawers and editorial forms in `components/FormLocaleSwitch.astro`: hidden with 1 or fewer site languages, sigla buttons (`data-drawer-lang-btn`) with 2, and a daisyUI select (`data-drawer-lang-select`) with 3+. Bind both controls through a `setDrawerLang(code)` helper that toggles button active states, syncs the select value, and switches `data-drawer-locale-panel` visibility; enumerate per-locale fields from the panels, never from the buttons.
