...
- Keep permission assignment in `components/PermissionMatrix.astro`: every module action has a Parent checkbox and a numeric Limit. An empty Limit means all matching records. Persist those values through `lib/permission-grants.ts`.
