...
- `0039_projects_module.sql` re-registers the pre-existing `proyectos` admin module idempotently (first registered by `0010`), creates `projects` with unique slugs, organization, workflow stage (`borrador`/`en_revision`/`aprobado`) and budget columns, `project_locales` for localized names/deks/descriptions/OG, and `projects_page_locales` for `/proyectos` hero and banner. Seeds three example projects with Spanish/English locales; no `search_documents` seeding because public project detail pages are out of scope.
- `0040_entrepreneurs_module.sql` registers the `emprendedores` admin module, creates `entrepreneurs` with unique slugs, category (`moda`/`alimentos`/`belleza`/`editorial`/`arte`), location/founding/contact fields, and website URL, `entrepreneur_locales` for localized names/deks/stories/quotes/ficha labels/OG, `entrepreneur_offerings` + `entrepreneur_offering_locales` for products/services with per-locale technical-sheet specs (`specs_json`), and `entrepreneurs_page_locales` for `/emprendedores` hero and banner. Seeds the page copy, one example company (Tejidos Ubuntu) with Spanish/English locales, and two example offerings; no `search_documents` seeding.
- `0041_store_module.sql` registers the `tienda` admin module, creates `store_products` with unique slugs, category (`ebook`/`lamina`/`merch`/`descargable`), price/compare-at/free and downloadable flags, `store_product_locales` for localized names/deks/descriptions/specs/OG, and `store_page_locales` for `/tienda` hero and banner. Seeds three example products with Spanish/English locales; no `search_documents` seeding because public product detail pages are out of scope.
- `0042_comments_module.sql` creates `user_comments` for the comments moderation queue (`article_id` nullable with `ON DELETE SET NULL` plus `article_title` snapshot, `user_id` nullable with `ON DELETE SET NULL`, `author_name` defaulting to `Anónimo`, status `pending|approved|reported|rejected`, `admin_reply`), adds `(status, created_at DESC)` and `(article_id, created_at DESC)` indexes, re-ensures the pre-existing `comentarios` RBAC permissions/grants idempotently (module itself was registered by `0010`), and seeds four moderation-queue rows matching the `/admin/comentarios` mock (ids 1–4, staggered timestamps); no public comment form exists yet.
